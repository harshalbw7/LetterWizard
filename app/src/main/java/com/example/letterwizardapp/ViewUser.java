...
public class ViewUser extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String[] headers = {" Sr.No. ", "  UserName ", "  First & Last Name ", "  Email ID ", "  Mobile Number "};
        ...
        reference.addListenerForSingleValueEvent(new ValueEventListener() {
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for (DataSnapshot dataSnapshot : snapshot.getChildren()) {
                    user letter = dataSnapshot.getValue(user.class);
                    String documentKey = dataSnapshot.getKey();

                    TableRow dummy = new TableRow(ViewUser.this);
                    dummy.setPadding(0,10,0,20);
                    table.addView(dummy);

                    TableRow row = new TableRow(ViewUser.this);
                    row.setPadding(20,40,20,40);
                    row.setBackgroundResource(R.drawable.back);
                    row.setElevation(20f);
                    row.setGravity(Gravity.CENTER_VERTICAL);

                    TextView column0 = new TextView(ViewUser.this);
                    column0.setText("  " + String.valueOf(no) + ". ");
                    column0.setTextColor(getResources().getColor(android.R.color.black));
                    row.addView(column0);

                    TextView column1 = new TextView(ViewUser.this);
                    column1.setText("" + letter.getUserName() + "  ");
                    column1.setTextColor(getResources().getColor(android.R.color.black));
                    row.addView(column1);

                    TextView column2 = new TextView(ViewUser.this);
                    column2.setText("" + letter.getName() + "   ");
                    column2.setTextColor(getResources().getColor(android.R.color.black));
                    row.addView(column2);

                    TextView column3 = new TextView(ViewUser.this);
                    column3.setText("" + letter.getEmail() + "   ");
                    column3.setTextColor(getResources().getColor(android.R.color.black));
                    row.addView(column3);

                    TextView column4 = new TextView(ViewUser.this);
                    column4.setText("" + letter.getMobile() + "   ");
                    column4.setTextColor(getResources().getColor(android.R.color.black));
                    row.addView(column4);

                    ImageView column5 = new ImageView(ViewUser.this);
                    column5.setImageResource(R.drawable.delete);
                    column5.setOnClickListener(v1 -> {
                        AlertDialog.Builder builder = new AlertDialog.Builder(ViewUser.this,R.style.CustomAlertDialogStyle);
                        builder.setTitle("Delete User");
                        builder.setMessage("Are you sure to delete ?");

                        builder.setPositiveButton("Yes", (dialog, which) -> {
                            assert documentKey != null;
                            DatabaseReference reference = db.getReference("user").child(documentKey);
                            reference.removeValue();
                            table.removeView(row);
                            Toast.makeText(ViewUser.this, "User deleted successfully !", Toast.LENGTH_SHORT).show();
                        });
                        ...
                        if (positiveButton != null && negativeButton != null) {
                            positiveButton.setTextColor(ContextCompat.getColor(ViewUser.this,R.color.black));
                            negativeButton.setTextColor(ContextCompat.getColor(ViewUser.this,R.color.black));
                        }
                    });
                    row.addView(column5);
                    table.addView(row);
        ...
        manageContent.setOnClickListener(v -> {
            Intent intent = new Intent(ViewUser.this,ManageContent.class);
        ...
        letterDetails.setOnClickListener(v -> {
            Intent intent = new Intent(ViewUser.this, letterDetails.class);
            intent.putExtra("USERNAME", userName);
            intent.putExtra("EMAIL", email);
            intent.putExtra("PASSWORD", password);
            startActivity(intent);
        ...
        viewUser.setTextColor(Color.parseColor("#D2D2D2"));

        viewFeedback.setOnClickListener(v -> {
            Intent intent = new Intent(ViewUser.this,ViewFeedbacks.class);
            intent.putExtra("USERNAME", userName);
            intent.putExtra("EMAIL", email);
            intent.putExtra("PASSWORD", password);
            startActivity(intent);
